
//#region src/core/fn.js
var ColorHandle = class {
	/**
	
	* 随机返回一个颜色值
	
	* @param {number} type - 0: HEX, 1: RGB, 2: HSL, 3: HWB
	
	* @returns {string} 颜色值
	
	*/
	randomColor(type = 0) {
		return type === 1 ? this.randomRGB() : type === 2 ? this.randomHSL() : type === 3 ? this.randomHWB() : this.randomHEX();
	}
	/**
	
	* 获取随机HEX色值
	
	* HEX色值是指采用16进制表示颜色的方法，使用3个或6个数字来表示颜色的红、绿、蓝三个通道的数值。
	
	* @returns {string} HEX色值
	
	*/
	randomHEX() {
		const letters = "0123456789ABCDEF";
		let color = "#";
		for (let i = 0; i < 6; i++) color += letters[Math.floor(Math.random() * 16)];
		return color;
	}
	/**
	
	* 随机返回RGB色值
	
	* @param {number} [R] - 红色分量 (0-255)
	
	* @param {number} [G] - 绿色分量 (0-255)
	
	* @param {number} [B] - 蓝色分量 (0-255)
	
	* @param {number} [A] - 透明度 (0-1)
	
	* @returns {string} RGB或RGBA色值
	
	*/
	randomRGB(R, G, B, A) {
		const args = [...arguments];
		if (args.length === 1) A = args[0];
		else if (args.length === 3) [R, G, B] = args;
		let r = R !== void 0 ? R : Math.floor(Math.random() * 256);
		let g = G !== void 0 ? G : Math.floor(Math.random() * 256);
		let b = B !== void 0 ? B : Math.floor(Math.random() * 256);
		if (A !== void 0) return `rgba(${r},${g},${b},${A})`;
		return `rgb(${r},${g},${b})`;
	}
	/**
	
	* 返回一个RGBA的色值（基于随机RGB，指定透明度）
	
	* @param {number} A - 透明度 (0-1)
	
	* @returns {string} RGBA色值
	
	*/
	randomRGBA(A) {
		return this.randomRGB(void 0, void 0, void 0, A);
	}
	/**
	
	* 随机返回HWB色值
	
	* HWB是色调（Hue）、白度（White）和黑度（Black）三个参数组成
	
	* @returns {string} HWB色值
	
	*/
	randomHWB() {
		const h = Math.floor(Math.random() * 360);
		const w = Math.floor(Math.random() * 100);
		const b = Math.floor(Math.random() * 100);
		return `hwb(${h},${w}%,${b}%)`;
	}
	/**
	
	* 随机返回HSL色值
	
	* HSL是色调（Hue）、饱和度（Saturation）和亮度（Lightness）三个参数组成
	
	* @returns {string} HSL色值
	
	*/
	randomHSL() {
		const h = Math.floor(Math.random() * 360);
		const s = Math.floor(Math.random() * 100);
		const l = Math.floor(Math.random() * 100);
		return `hsl(${h},${s}%,${l}%)`;
	}
	/**
	
	* RGB色值 => HEX色值
	
	* @param {number} r - 红色分量 (0-255)
	
	* @param {number} g - 绿色分量 (0-255)
	
	* @param {number} b - 蓝色分量 (0-255)
	
	* @returns {string} HEX色值
	
	*/
	rgbToHex(r, g, b) {
		const toHex = (n) => {
			const hex = n.toString(16).toUpperCase();
			return hex.length === 1 ? "0" + hex : hex;
		};
		return "#" + toHex(r) + toHex(g) + toHex(b);
	}
	/**
	
	* HEX色值 => RGB色值
	
	* @param {string} hex - HEX颜色字符串，如 "#FFF" 或 "#FFFFFF"
	
	* @returns {string|false} 返回RGB色值或 false（如果格式错误）
	
	*/
	hexToRgb(hex) {
		const pattern = /^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$/;
		if (!pattern.test(hex)) return false;
		if (hex.length === 4) {
			const h = hex.slice(1);
			hex = "#" + h[0] + h[0] + h[1] + h[1] + h[2] + h[2];
		}
		const r = parseInt(hex.slice(1, 3), 16);
		const g = parseInt(hex.slice(3, 5), 16);
		const b = parseInt(hex.slice(5, 7), 16);
		return `rgb(${r},${g},${b})`;
	}
};
var fn_default = ColorHandle;

//#endregion
//#region src/core/colors.js
var colors_default = colors = [
	{
		"name": "乳白",
		"hex": "#f9f4dc",
		"pinyin": "rubai"
	},
	{
		"name": "杏仁黄",
		"hex": "#f7e8aa",
		"pinyin": "xingrenhuang"
	},
	{
		"name": "茉莉黄",
		"hex": "#f8df72",
		"pinyin": "molihuang"
	},
	{
		"name": "麦秆黄",
		"hex": "#f8df70",
		"pinyin": "maiganhuang"
	},
	{
		"name": "油菜花黄",
		"hex": "#fbda41",
		"pinyin": "youcaihuahuang"
	},
	{
		"name": "佛手黄",
		"hex": "#fed71a",
		"pinyin": "foshouhuang"
	},
	{
		"name": "篾黄",
		"hex": "#f7de98",
		"pinyin": "miehuang"
	},
	{
		"name": "葵扇黄",
		"hex": "#f8d86a",
		"pinyin": "kuishanhuang"
	},
	{
		"name": "柠檬黄",
		"hex": "#fcd337",
		"pinyin": "ningmenghuang"
	},
	{
		"name": "金瓜黄",
		"hex": "#fcd217",
		"pinyin": "jinguahuang"
	},
	{
		"name": "藤黄",
		"hex": "#ffd111",
		"pinyin": "tenghuang"
	},
	{
		"name": "酪黄",
		"hex": "#f6dead",
		"pinyin": "laohuang"
	},
	{
		"name": "香水玫瑰黄",
		"hex": "#f7da94",
		"pinyin": "xiangshuimeiguihuang"
	},
	{
		"name": "淡密黄",
		"hex": "#f9d367",
		"pinyin": "danmihuang"
	},
	{
		"name": "大豆黄",
		"hex": "#fbcd31",
		"pinyin": "dadouhuang"
	},
	{
		"name": "素馨黄",
		"hex": "#fccb16",
		"pinyin": "suxinhuang"
	},
	{
		"name": "向日葵黄",
		"hex": "#fecc11",
		"pinyin": "xiangrikuihuang"
	},
	{
		"name": "雅梨黄",
		"hex": "#fbc82f",
		"pinyin": "yalihuang"
	},
	{
		"name": "黄连黄",
		"hex": "#fcc515",
		"pinyin": "huanglianhuang"
	},
	{
		"name": "金盏黄",
		"hex": "#fcc307",
		"pinyin": "jinzhanhuang"
	},
	{
		"name": "蛋壳黄",
		"hex": "#f8c387",
		"pinyin": "dankehuang"
	},
	{
		"name": "肉色",
		"hex": "#f7c173",
		"pinyin": "rouse"
	},
	{
		"name": "鹅掌黄",
		"hex": "#fbb929",
		"pinyin": "ezhanghuang"
	},
	{
		"name": "鸡蛋黄",
		"hex": "#fbb612",
		"pinyin": "jidanhuang"
	},
	{
		"name": "鼬黄",
		"hex": "#fcb70a",
		"pinyin": "youhuang"
	},
	{
		"name": "榴萼黄",
		"hex": "#f9a633",
		"pinyin": "liuehuang"
	},
	{
		"name": "淡橘橙",
		"hex": "#fba414",
		"pinyin": "danjucheng"
	},
	{
		"name": "枇杷黄",
		"hex": "#fca106",
		"pinyin": "pipahuang"
	},
	{
		"name": "橙皮黄",
		"hex": "#fca104",
		"pinyin": "chengpihuang"
	},
	{
		"name": "北瓜黄",
		"hex": "#fc8c23",
		"pinyin": "beiguahuang"
	},
	{
		"name": "杏黄",
		"hex": "#f28e16",
		"pinyin": "xinghuang"
	},
	{
		"name": "雄黄",
		"hex": "#ff9900",
		"pinyin": "xionghuang"
	},
	{
		"name": "万寿菊黄",
		"hex": "#fb8b05",
		"pinyin": "wanshoujuhuang"
	},
	{
		"name": "菊蕾白",
		"hex": "#e9ddb6",
		"pinyin": "juleibai"
	},
	{
		"name": "秋葵黄",
		"hex": "#eed045",
		"pinyin": "qiukuihuang"
	},
	{
		"name": "硫华黄",
		"hex": "#f2ce2b",
		"pinyin": "liuhuahuang"
	},
	{
		"name": "柚黄",
		"hex": "#f1ca17",
		"pinyin": "youhuang"
	},
	{
		"name": "芒果黄",
		"hex": "#ddc871",
		"pinyin": "mangguohuang"
	},
	{
		"name": "蒿黄",
		"hex": "#dfc243",
		"pinyin": "haohuang"
	},
	{
		"name": "姜黄",
		"hex": "#e2c027",
		"pinyin": "jianghuang"
	},
	{
		"name": "香蕉黄",
		"hex": "#e4bf11",
		"pinyin": "xiangjiaohuang"
	},
	{
		"name": "草黄",
		"hex": "#d2b42c",
		"pinyin": "caohuang"
	},
	{
		"name": "新禾绿",
		"hex": "#d2b116",
		"pinyin": "xinhelv"
	},
	{
		"name": "月灰",
		"hex": "#b7ae8f",
		"pinyin": "yuehui"
	},
	{
		"name": "淡灰绿",
		"hex": "#ad9e5f",
		"pinyin": "danhuilv"
	},
	{
		"name": "草灰绿",
		"hex": "#8e804b",
		"pinyin": "caohuilv"
	},
	{
		"name": "苔绿",
		"hex": "#887322",
		"pinyin": "tailv"
	},
	{
		"name": "碧螺春绿",
		"hex": "#867018",
		"pinyin": "biluochunlv"
	},
	{
		"name": "燕羽灰",
		"hex": "#685e48",
		"pinyin": "yanyuhui"
	},
	{
		"name": "蟹壳灰",
		"hex": "#695e45",
		"pinyin": "xiekehui"
	},
	{
		"name": "潭水绿",
		"hex": "#645822",
		"pinyin": "tanshuilv"
	},
	{
		"name": "橄榄绿",
		"hex": "#5e5314",
		"pinyin": "ganlanlv"
	},
	{
		"name": "蚌肉白",
		"hex": "#f9f1db",
		"pinyin": "bangroubai"
	},
	{
		"name": "豆汁黄",
		"hex": "#f8e8c1",
		"pinyin": "douzhihuang"
	},
	{
		"name": "淡茧黄",
		"hex": "#f9d770",
		"pinyin": "danjianhuang"
	},
	{
		"name": "乳鸭黄",
		"hex": "#ffc90c",
		"pinyin": "ruyahuang"
	},
	{
		"name": "荔肉白",
		"hex": "#f2e6ce",
		"pinyin": "liroubai"
	},
	{
		"name": "象牙黄",
		"hex": "#f0d695",
		"pinyin": "xiangyahuang"
	},
	{
		"name": "炒米黄",
		"hex": "#f4ce69",
		"pinyin": "chaomihuang"
	},
	{
		"name": "鹦鹉冠黄",
		"hex": "#f6c430",
		"pinyin": "yingwuguanhuang"
	},
	{
		"name": "木瓜黄",
		"hex": "#f9c116",
		"pinyin": "muguahuang"
	},
	{
		"name": "浅烙黄",
		"hex": "#f9bd10",
		"pinyin": "qianlaohuang"
	},
	{
		"name": "莲子白",
		"hex": "#e5d3aa",
		"pinyin": "lianzibai"
	},
	{
		"name": "谷黄",
		"hex": "#e8b004",
		"pinyin": "guhuang"
	},
	{
		"name": "栀子黄",
		"hex": "#ebb10d",
		"pinyin": "zhizihuang"
	},
	{
		"name": "芥黄",
		"hex": "#d9a40e",
		"pinyin": "jiehuang"
	},
	{
		"name": "银鼠灰",
		"hex": "#b5aa90",
		"pinyin": "yinshuhui"
	},
	{
		"name": "尘灰",
		"hex": "#b6a476",
		"pinyin": "chenhui"
	},
	{
		"name": "枯绿",
		"hex": "#b78d12",
		"pinyin": "kulv"
	},
	{
		"name": "鲛青",
		"hex": "#87723e",
		"pinyin": "jiaoqing"
	},
	{
		"name": "粽叶绿",
		"hex": "#876818",
		"pinyin": "zongyelv"
	},
	{
		"name": "灰绿",
		"hex": "#8a6913",
		"pinyin": "huilv"
	},
	{
		"name": "鹤灰",
		"hex": "#4a4035",
		"pinyin": "hehui"
	},
	{
		"name": "淡松烟",
		"hex": "#4d4030",
		"pinyin": "dansongyan"
	},
	{
		"name": "暗海水绿",
		"hex": "#584717",
		"pinyin": "anhaishuilv"
	},
	{
		"name": "棕榈绿",
		"hex": "#5b4913",
		"pinyin": "zonglvlv"
	},
	{
		"name": "米色",
		"hex": "#f9e9cd",
		"pinyin": "mise"
	},
	{
		"name": "淡肉色",
		"hex": "#f8e0b0",
		"pinyin": "danrouse"
	},
	{
		"name": "麦芽糖黄",
		"hex": "#f9d27d",
		"pinyin": "maiyatanghuang"
	},
	{
		"name": "琥珀黄",
		"hex": "#feba07",
		"pinyin": "hupohuang"
	},
	{
		"name": "甘草黄",
		"hex": "#f3bf4c",
		"pinyin": "gancaohuang"
	},
	{
		"name": "初熟杏黄",
		"hex": "#f8bc31",
		"pinyin": "chushuxinghuang"
	},
	{
		"name": "浅驼色",
		"hex": "#e2c17c",
		"pinyin": "qiantuose"
	},
	{
		"name": "沙石黄",
		"hex": "#e5b751",
		"pinyin": "shashihuang"
	},
	{
		"name": "虎皮黄",
		"hex": "#eaad1a",
		"pinyin": "hupihuang"
	},
	{
		"name": "土黄",
		"hex": "#d6a01d",
		"pinyin": "tuhuang"
	},
	{
		"name": "百灵鸟灰",
		"hex": "#b4a992",
		"pinyin": "bailingniaohui"
	},
	{
		"name": "山鸡黄",
		"hex": "#b78b26",
		"pinyin": "shanjihuang"
	},
	{
		"name": "龟背黄",
		"hex": "#826b48",
		"pinyin": "guibeihuang"
	},
	{
		"name": "苍黄",
		"hex": "#806332",
		"pinyin": "canghuang"
	},
	{
		"name": "莱阳梨黄",
		"hex": "#815f25",
		"pinyin": "laiyanglihuang"
	},
	{
		"name": "蜴蜊绿",
		"hex": "#835e1d",
		"pinyin": "yililv"
	},
	{
		"name": "松鼠灰",
		"hex": "#4f4032",
		"pinyin": "songshuhui"
	},
	{
		"name": "橄榄灰",
		"hex": "#503e2a",
		"pinyin": "ganlanhui"
	},
	{
		"name": "蟹壳绿",
		"hex": "#513c20",
		"pinyin": "xiekelv"
	},
	{
		"name": "古铜绿",
		"hex": "#533c1b",
		"pinyin": "gutonglv"
	},
	{
		"name": "焦茶绿",
		"hex": "#553b18",
		"pinyin": "jiaochalv"
	},
	{
		"name": "粉白",
		"hex": "#fbf2e3",
		"pinyin": "fenbai"
	},
	{
		"name": "落英淡粉",
		"hex": "#f9e8d0",
		"pinyin": "luoyingdanfen"
	},
	{
		"name": "瓜瓤粉",
		"hex": "#f9cb8b",
		"pinyin": "guarangfen"
	},
	{
		"name": "蜜黄",
		"hex": "#fbb957",
		"pinyin": "mihuang"
	},
	{
		"name": "金叶黄",
		"hex": "#ffa60f",
		"pinyin": "jinyehuang"
	},
	{
		"name": "金莺黄",
		"hex": "#f4a83a",
		"pinyin": "jinyinghuang"
	},
	{
		"name": "鹿角棕",
		"hex": "#e3bd8d",
		"pinyin": "lujiaozong"
	},
	{
		"name": "凋叶棕",
		"hex": "#e7a23f",
		"pinyin": "diaoyezong"
	},
	{
		"name": "玳瑁黄",
		"hex": "#daa45a",
		"pinyin": "daimaohuang"
	},
	{
		"name": "软木黄",
		"hex": "#de9e44",
		"pinyin": "ruanmuhuang"
	},
	{
		"name": "风帆黄",
		"hex": "#dc9123",
		"pinyin": "fengfanhuang"
	},
	{
		"name": "桂皮淡棕",
		"hex": "#c09351",
		"pinyin": "guipidanzong"
	},
	{
		"name": "猴毛灰",
		"hex": "#97846c",
		"pinyin": "houmaohui"
	},
	{
		"name": "山鸡褐",
		"hex": "#986524",
		"pinyin": "shanjihe"
	},
	{
		"name": "驼色",
		"hex": "#66462a",
		"pinyin": "tuose"
	},
	{
		"name": "茶褐",
		"hex": "#5d3d21",
		"pinyin": "chahe"
	},
	{
		"name": "古铜褐",
		"hex": "#5c3719",
		"pinyin": "gutonghe"
	},
	{
		"name": "荷花白",
		"hex": "#fbecde",
		"pinyin": "hehuabai"
	},
	{
		"name": "玫瑰粉",
		"hex": "#f8b37f",
		"pinyin": "meiguifen"
	},
	{
		"name": "橘橙",
		"hex": "#f97d1c",
		"pinyin": "jucheng"
	},
	{
		"name": "美人焦橙",
		"hex": "#fa7e23",
		"pinyin": "meirenjiaocheng"
	},
	{
		"name": "润红",
		"hex": "#f7cdbc",
		"pinyin": "runhong"
	},
	{
		"name": "淡桃红",
		"hex": "#f6cec1",
		"pinyin": "dantaohong"
	},
	{
		"name": "海螺橙",
		"hex": "#f0945d",
		"pinyin": "hailuocheng"
	},
	{
		"name": "桃红",
		"hex": "#f0ada0",
		"pinyin": "taohong"
	},
	{
		"name": "颊红",
		"hex": "#eeaa9c",
		"pinyin": "jiahong"
	},
	{
		"name": "淡罂粟红",
		"hex": "#eea08c",
		"pinyin": "danyingsuhong"
	},
	{
		"name": "晨曦红",
		"hex": "#ea8958",
		"pinyin": "chenxihong"
	},
	{
		"name": "蟹壳红",
		"hex": "#f27635",
		"pinyin": "xiekehong"
	},
	{
		"name": "金莲花橙",
		"hex": "#f86b1d",
		"pinyin": "jinlianhuacheng"
	},
	{
		"name": "草莓红",
		"hex": "#ef6f48",
		"pinyin": "caomeihong"
	},
	{
		"name": "龙睛鱼红",
		"hex": "#ef632b",
		"pinyin": "longjingyuhong"
	},
	{
		"name": "蜻蜓红",
		"hex": "#f1441d",
		"pinyin": "qingtinghong"
	},
	{
		"name": "大红",
		"hex": "#f04b22",
		"pinyin": "dahong"
	},
	{
		"name": "柿红",
		"hex": "#f2481b",
		"pinyin": "shihong"
	},
	{
		"name": "榴花红",
		"hex": "#f34718",
		"pinyin": "liuhuahong"
	},
	{
		"name": "银朱",
		"hex": "#f43e06",
		"pinyin": "yinzhu"
	},
	{
		"name": "朱红",
		"hex": "#ed5126",
		"pinyin": "zhuhong"
	},
	{
		"name": "鲑鱼红",
		"hex": "#f09c5a",
		"pinyin": "guiyuhong"
	},
	{
		"name": "金黄",
		"hex": "#f26b1f",
		"pinyin": "jinhuang"
	},
	{
		"name": "鹿皮褐",
		"hex": "#d99156",
		"pinyin": "lupihe"
	},
	{
		"name": "醉瓜肉",
		"hex": "#db8540",
		"pinyin": "zuiguarou"
	},
	{
		"name": "麂棕",
		"hex": "#de7622",
		"pinyin": "jizong"
	},
	{
		"name": "淡银灰",
		"hex": "#c1b2a3",
		"pinyin": "danyinhui"
	},
	{
		"name": "淡赭",
		"hex": "#be7e4a",
		"pinyin": "danzhe"
	},
	{
		"name": "槟榔综",
		"hex": "#c1651a",
		"pinyin": "binglangzong"
	},
	{
		"name": "银灰",
		"hex": "#918072",
		"pinyin": "yinhui"
	},
	{
		"name": "海鸥灰",
		"hex": "#9a8878",
		"pinyin": "haiouhui"
	},
	{
		"name": "淡咖啡",
		"hex": "#945833",
		"pinyin": "dankafei"
	},
	{
		"name": "岩石棕",
		"hex": "#964d22",
		"pinyin": "yanshizong"
	},
	{
		"name": "芒果棕",
		"hex": "#954416",
		"pinyin": "mangguozong"
	},
	{
		"name": "石板灰",
		"hex": "#624941",
		"pinyin": "shibanhui"
	},
	{
		"name": "珠母灰",
		"hex": "#64483d",
		"pinyin": "zhumuhui"
	},
	{
		"name": "丁香棕",
		"hex": "#71361d",
		"pinyin": "dingxiangzong"
	},
	{
		"name": "咖啡",
		"hex": "#753117",
		"pinyin": "kafei"
	},
	{
		"name": "筍皮棕",
		"hex": "#732e12",
		"pinyin": "sunpizong"
	},
	{
		"name": "燕颔红",
		"hex": "#fc6315",
		"pinyin": "yanhanhong"
	},
	{
		"name": "玉粉红",
		"hex": "#e8b49a",
		"pinyin": "yufenhong"
	},
	{
		"name": "金驼",
		"hex": "#e46828",
		"pinyin": "jintuo"
	},
	{
		"name": "铁棕",
		"hex": "#d85916",
		"pinyin": "tiezong"
	},
	{
		"name": "蛛网灰",
		"hex": "#b7a091",
		"pinyin": "zhuwanghui"
	},
	{
		"name": "淡可可棕",
		"hex": "#b7511d",
		"pinyin": "dankekezong"
	},
	{
		"name": "中红灰",
		"hex": "#8b614d",
		"pinyin": "zhonghonghui"
	},
	{
		"name": "淡土黄",
		"hex": "#8c4b31",
		"pinyin": "dantuhuang"
	},
	{
		"name": "淡豆沙",
		"hex": "#873d24",
		"pinyin": "dandousha"
	},
	{
		"name": "椰壳棕",
		"hex": "#883a1e",
		"pinyin": "yekezong"
	},
	{
		"name": "淡铁灰",
		"hex": "#5b423a",
		"pinyin": "dantiehui"
	},
	{
		"name": "中灰驼",
		"hex": "#603d30",
		"pinyin": "zhonghuituo"
	},
	{
		"name": "淡栗棕",
		"hex": "#673424",
		"pinyin": "danlizong"
	},
	{
		"name": "可可棕",
		"hex": "#652b1c",
		"pinyin": "kekezong"
	},
	{
		"name": "柞叶棕",
		"hex": "#692a1b",
		"pinyin": "zhayezong"
	},
	{
		"name": "野蔷薇红",
		"hex": "#fb9968",
		"pinyin": "yeqiangweihong"
	},
	{
		"name": "菠萝红",
		"hex": "#fc7930",
		"pinyin": "boluohong"
	},
	{
		"name": "藕荷",
		"hex": "#edc3ae",
		"pinyin": "ouhe"
	},
	{
		"name": "陶瓷红",
		"hex": "#e16723",
		"pinyin": "taocihong"
	},
	{
		"name": "晓灰",
		"hex": "#d4c4b7",
		"pinyin": "xiaohui"
	},
	{
		"name": "余烬红",
		"hex": "#cf7543",
		"pinyin": "yujinhong"
	},
	{
		"name": "火砖红",
		"hex": "#cd6227",
		"pinyin": "huozhuanhong"
	},
	{
		"name": "火泥棕",
		"hex": "#aa6a4c",
		"pinyin": "huonizong"
	},
	{
		"name": "绀红",
		"hex": "#a6522c",
		"pinyin": "ganhong"
	},
	{
		"name": "橡树棕",
		"hex": "#773d31",
		"pinyin": "xiangshuzong"
	},
	{
		"name": "海报灰",
		"hex": "#483332",
		"pinyin": "haibaohui"
	},
	{
		"name": "玫瑰灰",
		"hex": "#4b2e2b",
		"pinyin": "meiguihui"
	},
	{
		"name": "火山棕",
		"hex": "#482522",
		"pinyin": "huoshanzong"
	},
	{
		"name": "豆沙",
		"hex": "#481e1c",
		"pinyin": "dousha"
	},
	{
		"name": "淡米粉",
		"hex": "#fbeee2",
		"pinyin": "danmifen"
	},
	{
		"name": "初桃粉红",
		"hex": "#f6dcce",
		"pinyin": "chutaofenhong"
	},
	{
		"name": "介壳淡粉红",
		"hex": "#f7cfba",
		"pinyin": "jieqiaodanfenhong"
	},
	{
		"name": "淡藏花红",
		"hex": "#f6ad8f",
		"pinyin": "dancanghuahong"
	},
	{
		"name": "瓜瓤红",
		"hex": "#f68c60",
		"pinyin": "guaranghong"
	},
	{
		"name": "芙蓉红",
		"hex": "#f9723d",
		"pinyin": "furonghong"
	},
	{
		"name": "莓酱红",
		"hex": "#fa5d19",
		"pinyin": "meijianghong"
	},
	{
		"name": "法螺红",
		"hex": "#ee8055",
		"pinyin": "faluohong"
	},
	{
		"name": "落霞红",
		"hex": "#cf4813",
		"pinyin": "luoxiahong"
	},
	{
		"name": "淡玫瑰灰",
		"hex": "#b89485",
		"pinyin": "danmeiguihui"
	},
	{
		"name": "蟹蝥红",
		"hex": "#b14b28",
		"pinyin": "xiemaohong"
	},
	{
		"name": "火岩棕",
		"hex": "#863020",
		"pinyin": "huoyanzong"
	},
	{
		"name": "赭石",
		"hex": "#862617",
		"pinyin": "zheshi"
	},
	{
		"name": "暗驼棕",
		"hex": "#592620",
		"pinyin": "antuozong"
	},
	{
		"name": "酱棕",
		"hex": "#5a1f1b",
		"pinyin": "jiangzong"
	},
	{
		"name": "栗棕",
		"hex": "#5c1e19",
		"pinyin": "lizong"
	},
	{
		"name": "洋水仙红",
		"hex": "#f4c7ba",
		"pinyin": "yangshuixianhong"
	},
	{
		"name": "谷鞘红",
		"hex": "#f17666",
		"pinyin": "guqiaohong"
	},
	{
		"name": "苹果红",
		"hex": "#f15642",
		"pinyin": "pingguohong"
	},
	{
		"name": "铁水红",
		"hex": "#f5391c",
		"pinyin": "tieshuihong"
	},
	{
		"name": "桂红",
		"hex": "#f25a47",
		"pinyin": "guihong"
	},
	{
		"name": "极光红",
		"hex": "#f33b1f",
		"pinyin": "jiguanghong"
	},
	{
		"name": "粉红",
		"hex": "#f2b9b2",
		"pinyin": "fenhong"
	},
	{
		"name": "舌红",
		"hex": "#f19790",
		"pinyin": "shehong"
	},
	{
		"name": "曲红",
		"hex": "#f05a46",
		"pinyin": "quhong"
	},
	{
		"name": "红汞红",
		"hex": "#f23e23",
		"pinyin": "honggonghong"
	},
	{
		"name": "淡绯",
		"hex": "#f2cac9",
		"pinyin": "danfei"
	},
	{
		"name": "无花果红",
		"hex": "#efafad",
		"pinyin": "wuhuaguohong"
	},
	{
		"name": "榴子红",
		"hex": "#f1908c",
		"pinyin": "liuzihong"
	},
	{
		"name": "胭脂红",
		"hex": "#f03f24",
		"pinyin": "yanzhihong"
	},
	{
		"name": "合欢红",
		"hex": "#f0a1a8",
		"pinyin": "hehuanhong"
	},
	{
		"name": "春梅红",
		"hex": "#f1939c",
		"pinyin": "chunmeihong"
	},
	{
		"name": "香叶红",
		"hex": "#f07c82",
		"pinyin": "xiangyehong"
	},
	{
		"name": "珊瑚红",
		"hex": "#f04a3a",
		"pinyin": "shanhuhong"
	},
	{
		"name": "萝卜红",
		"hex": "#f13c22",
		"pinyin": "luobohong"
	},
	{
		"name": "淡茜红",
		"hex": "#e77c8e",
		"pinyin": "danqianhong"
	},
	{
		"name": "艳红",
		"hex": "#ed5a65",
		"pinyin": "yanhong"
	},
	{
		"name": "淡菽红",
		"hex": "#ed4845",
		"pinyin": "danshuhong"
	},
	{
		"name": "鱼鳃红",
		"hex": "#ed3b2f",
		"pinyin": "yusaihong"
	},
	{
		"name": "樱桃红",
		"hex": "#ed3321",
		"pinyin": "yingtaohong"
	},
	{
		"name": "淡蕊香红",
		"hex": "#ee4866",
		"pinyin": "danruixianghong"
	},
	{
		"name": "石竹红",
		"hex": "#ee4863",
		"pinyin": "shizhuhong"
	},
	{
		"name": "草茉莉红",
		"hex": "#ef475d",
		"pinyin": "caomolihong"
	},
	{
		"name": "茶花红",
		"hex": "#ee3f4d",
		"pinyin": "chahuahong"
	},
	{
		"name": "枸枢红",
		"hex": "#ed3333",
		"pinyin": "goushuhong"
	},
	{
		"name": "秋海棠红",
		"hex": "#ec2b24",
		"pinyin": "qiuhaitanghong"
	},
	{
		"name": "丽春红",
		"hex": "#eb261a",
		"pinyin": "lichunhong"
	},
	{
		"name": "夕阳红",
		"hex": "#de2a18",
		"pinyin": "xiyanghong"
	},
	{
		"name": "鹤顶红",
		"hex": "#d42517",
		"pinyin": "hedinghong"
	},
	{
		"name": "鹅血石红",
		"hex": "#ab372f",
		"pinyin": "exueshihong"
	},
	{
		"name": "覆盆子红",
		"hex": "#ac1f18",
		"pinyin": "fupenzihong"
	},
	{
		"name": "貂紫",
		"hex": "#5d3131",
		"pinyin": "diaozi"
	},
	{
		"name": "暗玉紫",
		"hex": "#5c2223",
		"pinyin": "anyuzi"
	},
	{
		"name": "栗紫",
		"hex": "#5a191b",
		"pinyin": "lizi"
	},
	{
		"name": "葡萄酱紫",
		"hex": "#5a1216",
		"pinyin": "putaojiangzi"
	},
	{
		"name": "牡丹粉红",
		"hex": "#eea2a4",
		"pinyin": "mudanfenhong"
	},
	{
		"name": "山茶红",
		"hex": "#ed556a",
		"pinyin": "shanchahong"
	},
	{
		"name": "海棠红",
		"hex": "#f03752",
		"pinyin": "haitanghong"
	},
	{
		"name": "玉红",
		"hex": "#c04851",
		"pinyin": "yuhong"
	},
	{
		"name": "高粱红",
		"hex": "#c02c38",
		"pinyin": "gaolianghong"
	},
	{
		"name": "满江红",
		"hex": "#a7535a",
		"pinyin": "manjianghong"
	},
	{
		"name": "枣红",
		"hex": "#7c1823",
		"pinyin": "zaohong"
	},
	{
		"name": "葡萄紫",
		"hex": "#4c1f24",
		"pinyin": "putaozi"
	},
	{
		"name": "酱紫",
		"hex": "#4d1018",
		"pinyin": "jiangzi"
	},
	{
		"name": "淡曙红",
		"hex": "#ee2746",
		"pinyin": "danshuhong"
	},
	{
		"name": "唐菖蒲红",
		"hex": "#de1c31",
		"pinyin": "tangchangpuhong"
	},
	{
		"name": "鹅冠红",
		"hex": "#d11a2d",
		"pinyin": "eguanhong"
	},
	{
		"name": "莓红",
		"hex": "#c45a65",
		"pinyin": "meihong"
	},
	{
		"name": "枫叶红",
		"hex": "#c21f30",
		"pinyin": "fengyehong"
	},
	{
		"name": "苋菜红",
		"hex": "#a61b29",
		"pinyin": "xiancaihong"
	},
	{
		"name": "烟红",
		"hex": "#894e54",
		"pinyin": "yanhong"
	},
	{
		"name": "暗紫苑红",
		"hex": "#82202b",
		"pinyin": "anziyuanhong"
	},
	{
		"name": "殷红",
		"hex": "#82111f",
		"pinyin": "yanhong"
	},
	{
		"name": "猪肝紫",
		"hex": "#541e24",
		"pinyin": "zhuganzi"
	},
	{
		"name": "金鱼紫",
		"hex": "#500a16",
		"pinyin": "jinyuzi"
	},
	{
		"name": "草珠红",
		"hex": "#f8ebe6",
		"pinyin": "caozhuhong"
	},
	{
		"name": "淡绛红",
		"hex": "#ec7696",
		"pinyin": "danjianghong"
	},
	{
		"name": "品红",
		"hex": "#ef3473",
		"pinyin": "pinhong"
	},
	{
		"name": "凤仙花红",
		"hex": "#ea7293",
		"pinyin": "fengxianhuahong"
	},
	{
		"name": "粉团花红",
		"hex": "#ec9bad",
		"pinyin": "fentuanhuahong"
	},
	{
		"name": "夹竹桃红",
		"hex": "#eb507e",
		"pinyin": "jiazhutaohong"
	},
	{
		"name": "榲桲红",
		"hex": "#ed2f6a",
		"pinyin": "wenpohong"
	},
	{
		"name": "姜红",
		"hex": "#eeb8c3",
		"pinyin": "jianghong"
	},
	{
		"name": "莲瓣红",
		"hex": "#ea517f",
		"pinyin": "lianbanhong"
	},
	{
		"name": "水红",
		"hex": "#f1c4cd",
		"pinyin": "shuihong"
	},
	{
		"name": "报春红",
		"hex": "#ec8aa4",
		"pinyin": "baochunhong"
	},
	{
		"name": "月季红",
		"hex": "#ce5777",
		"pinyin": "yuejihong"
	},
	{
		"name": "豇豆红",
		"hex": "#ed9db2",
		"pinyin": "jiangdouhong"
	},
	{
		"name": "霞光红",
		"hex": "#ef82a0",
		"pinyin": "xiaguanghong"
	},
	{
		"name": "松叶牡丹红",
		"hex": "#eb3c70",
		"pinyin": "songyemudanhong"
	},
	{
		"name": "喜蛋红",
		"hex": "#ec2c64",
		"pinyin": "xidanhong"
	},
	{
		"name": "鼠鼻红",
		"hex": "#e3b4b8",
		"pinyin": "shubihong"
	},
	{
		"name": "尖晶玉红",
		"hex": "#cc163a",
		"pinyin": "jianjingyuhong"
	},
	{
		"name": "山黎豆红",
		"hex": "#c27c88",
		"pinyin": "shanlidouhong"
	},
	{
		"name": "锦葵红",
		"hex": "#bf3553",
		"pinyin": "jinkuihong"
	},
	{
		"name": "鼠背灰",
		"hex": "#73575c",
		"pinyin": "shubeihui"
	},
	{
		"name": "甘蔗紫",
		"hex": "#621624",
		"pinyin": "ganzhezi"
	},
	{
		"name": "石竹紫",
		"hex": "#63071c",
		"pinyin": "shizhuzi"
	},
	{
		"name": "苍蝇灰",
		"hex": "#36282b",
		"pinyin": "cangyinghui"
	},
	{
		"name": "卵石紫",
		"hex": "#30161c",
		"pinyin": "luanshizi"
	},
	{
		"name": "李紫",
		"hex": "#2b1216",
		"pinyin": "lizi"
	},
	{
		"name": "茄皮紫",
		"hex": "#2d0c13",
		"pinyin": "qiepizi"
	},
	{
		"name": "吊钟花红",
		"hex": "#ce5e8a",
		"pinyin": "diaozhonghuahong"
	},
	{
		"name": "兔眼红",
		"hex": "#ec4e8a",
		"pinyin": "tuyanhong"
	},
	{
		"name": "紫荆红",
		"hex": "#ee2c79",
		"pinyin": "zijinghong"
	},
	{
		"name": "菜头紫",
		"hex": "#951c48",
		"pinyin": "caitouzi"
	},
	{
		"name": "鹞冠紫",
		"hex": "#621d34",
		"pinyin": "yaoguanzi"
	},
	{
		"name": "葡萄酒红",
		"hex": "#62102e",
		"pinyin": "putaojiuhong"
	},
	{
		"name": "磨石紫",
		"hex": "#382129",
		"pinyin": "moshizi"
	},
	{
		"name": "檀紫",
		"hex": "#381924",
		"pinyin": "tanzi"
	},
	{
		"name": "火鹅紫",
		"hex": "#33141e",
		"pinyin": "huoezi"
	},
	{
		"name": "墨紫",
		"hex": "#310f1b",
		"pinyin": "mozi"
	},
	{
		"name": "晶红",
		"hex": "#eea6b7",
		"pinyin": "jinghong"
	},
	{
		"name": "扁豆花红",
		"hex": "#ef498b",
		"pinyin": "biandouhuahong"
	},
	{
		"name": "白芨红",
		"hex": "#de7897",
		"pinyin": "baijihong"
	},
	{
		"name": "嫩菱红",
		"hex": "#de3f7c",
		"pinyin": "nenlinghong"
	},
	{
		"name": "菠根红",
		"hex": "#d13c74",
		"pinyin": "bogenhong"
	},
	{
		"name": "酢酱草红",
		"hex": "#c5708b",
		"pinyin": "cujiangcaohong"
	},
	{
		"name": "洋葱紫",
		"hex": "#a8456b",
		"pinyin": "yangcongzi"
	},
	{
		"name": "海象紫",
		"hex": "#4b1e2f",
		"pinyin": "haixiangzi"
	},
	{
		"name": "绀紫",
		"hex": "#461629",
		"pinyin": "ganzi"
	},
	{
		"name": "古铜紫",
		"hex": "#440e25",
		"pinyin": "gutongzi"
	},
	{
		"name": "石蕊红",
		"hex": "#f0c9cf",
		"pinyin": "shiruihong"
	},
	{
		"name": "芍药耕红",
		"hex": "#eba0b3",
		"pinyin": "shaoyaogenghong"
	},
	{
		"name": "藏花红",
		"hex": "#ec2d7a",
		"pinyin": "canghuahong"
	},
	{
		"name": "初荷红",
		"hex": "#e16c96",
		"pinyin": "chuhehong"
	},
	{
		"name": "马鞭草紫",
		"hex": "#ede3e7",
		"pinyin": "mabiancaozi"
	},
	{
		"name": "丁香淡紫",
		"hex": "#e9d7df",
		"pinyin": "dingxiangdanzi"
	},
	{
		"name": "丹紫红",
		"hex": "#d2568c",
		"pinyin": "danzihong"
	},
	{
		"name": "玫瑰红",
		"hex": "#d2357d",
		"pinyin": "meiguihong"
	},
	{
		"name": "淡牵牛紫",
		"hex": "#d1c2d3",
		"pinyin": "danqianniuzi"
	},
	{
		"name": "凤信紫",
		"hex": "#c8adc4",
		"pinyin": "fengxinzi"
	},
	{
		"name": "萝兰紫",
		"hex": "#c08eaf",
		"pinyin": "luolanzi"
	},
	{
		"name": "玫瑰紫",
		"hex": "#ba2f7b",
		"pinyin": "meiguizi"
	},
	{
		"name": "藤萝紫",
		"hex": "#8076a3",
		"pinyin": "tengluozi"
	},
	{
		"name": "槿紫",
		"hex": "#806d9e",
		"pinyin": "jinzi"
	},
	{
		"name": "蕈紫",
		"hex": "#815c94",
		"pinyin": "xunzi"
	},
	{
		"name": "桔梗紫",
		"hex": "#813c85",
		"pinyin": "jiegengzi"
	},
	{
		"name": "魏紫",
		"hex": "#7e1671",
		"pinyin": "weizi"
	},
	{
		"name": "芝兰紫",
		"hex": "#e9ccd3",
		"pinyin": "zhilanzi"
	},
	{
		"name": "菱锰红",
		"hex": "#d276a3",
		"pinyin": "lingmenghong"
	},
	{
		"name": "龙须红",
		"hex": "#cc5595",
		"pinyin": "longxuhong"
	},
	{
		"name": "蓟粉红",
		"hex": "#e6d2d5",
		"pinyin": "jifenhong"
	},
	{
		"name": "电气石红",
		"hex": "#c35691",
		"pinyin": "dianqishihong"
	},
	{
		"name": "樱草紫",
		"hex": "#c06f98",
		"pinyin": "yingcaozi"
	},
	{
		"name": "芦穗灰",
		"hex": "#bdaead",
		"pinyin": "lusuihui"
	},
	{
		"name": "隐红灰",
		"hex": "#b598a1",
		"pinyin": "yinhonghui"
	},
	{
		"name": "苋菜紫",
		"hex": "#9b1e64",
		"pinyin": "xiancaizi"
	},
	{
		"name": "芦灰",
		"hex": "#856d72",
		"pinyin": "luhui"
	},
	{
		"name": "暮云灰",
		"hex": "#4f383e",
		"pinyin": "muyunhui"
	},
	{
		"name": "斑鸠灰",
		"hex": "#482936",
		"pinyin": "banjiuhui"
	},
	{
		"name": "淡藤萝紫",
		"hex": "#f2e7e5",
		"pinyin": "dantengluozi"
	},
	{
		"name": "淡青紫",
		"hex": "#e0c8d1",
		"pinyin": "danqingzi"
	},
	{
		"name": "青蛤壳紫",
		"hex": "#bc84a8",
		"pinyin": "qinghakezi"
	},
	{
		"name": "豆蔻紫",
		"hex": "#ad6598",
		"pinyin": "doukouzi"
	},
	{
		"name": "扁豆紫",
		"hex": "#a35c8f",
		"pinyin": "biandouzi"
	},
	{
		"name": "芥花紫",
		"hex": "#983680",
		"pinyin": "jiehuazi"
	},
	{
		"name": "青莲",
		"hex": "#8b2671",
		"pinyin": "qinglian"
	},
	{
		"name": "芓紫",
		"hex": "#894276",
		"pinyin": "zizi"
	},
	{
		"name": "葛巾紫",
		"hex": "#7e2065",
		"pinyin": "gejinzi"
	},
	{
		"name": "牵牛紫",
		"hex": "#681752",
		"pinyin": "qianniuzi"
	},
	{
		"name": "紫灰",
		"hex": "#5d3f51",
		"pinyin": "zihui"
	},
	{
		"name": "龙睛鱼紫",
		"hex": "#4e2a40",
		"pinyin": "longjingyuzi"
	},
	{
		"name": "荸荠紫",
		"hex": "#411c35",
		"pinyin": "biqizi"
	},
	{
		"name": "古鼎灰",
		"hex": "#36292f",
		"pinyin": "gudinghui"
	},
	{
		"name": "乌梅紫",
		"hex": "#1e131d",
		"pinyin": "wumeizi"
	},
	{
		"name": "深牵牛紫",
		"hex": "#1c0d1a",
		"pinyin": "shenqianniuzi"
	},
	{
		"name": "银白",
		"hex": "#f1f0ed",
		"pinyin": "yinbai"
	},
	{
		"name": "芡食白",
		"hex": "#e2e1e4",
		"pinyin": "qianshibai"
	},
	{
		"name": "远山紫",
		"hex": "#ccccd6",
		"pinyin": "yuanshanzi"
	},
	{
		"name": "淡蓝紫",
		"hex": "#a7a8bd",
		"pinyin": "danlanzi"
	},
	{
		"name": "山梗紫",
		"hex": "#61649f",
		"pinyin": "shangengzi"
	},
	{
		"name": "螺甸紫",
		"hex": "#74759b",
		"pinyin": "luodianzi"
	},
	{
		"name": "玛瑙灰",
		"hex": "#cfccc9",
		"pinyin": "manaohui"
	},
	{
		"name": "野菊紫",
		"hex": "#525288",
		"pinyin": "yejuzi"
	},
	{
		"name": "满天星紫",
		"hex": "#2e317c",
		"pinyin": "mantianxingzi"
	},
	{
		"name": "锌灰",
		"hex": "#7a7374",
		"pinyin": "xinhui"
	},
	{
		"name": "野葡萄紫",
		"hex": "#302f4b",
		"pinyin": "yeputaozi"
	},
	{
		"name": "剑锋紫",
		"hex": "#3e3841",
		"pinyin": "jianfengzi"
	},
	{
		"name": "龙葵紫",
		"hex": "#322f3b",
		"pinyin": "longkuizi"
	},
	{
		"name": "暗龙胆紫",
		"hex": "#22202e",
		"pinyin": "anlongdanzi"
	},
	{
		"name": "晶石紫",
		"hex": "#1f2040",
		"pinyin": "jingshizi"
	},
	{
		"name": "暗蓝紫",
		"hex": "#131124",
		"pinyin": "anlanzi"
	},
	{
		"name": "景泰蓝",
		"hex": "#2775b6",
		"pinyin": "jingtailan"
	},
	{
		"name": "尼罗蓝",
		"hex": "#2474b5",
		"pinyin": "niluolan"
	},
	{
		"name": "远天蓝",
		"hex": "#d0dfe6",
		"pinyin": "yuantianlan"
	},
	{
		"name": "星蓝",
		"hex": "#93b5cf",
		"pinyin": "xinglan"
	},
	{
		"name": "羽扇豆蓝",
		"hex": "#619ac3",
		"pinyin": "yushandoulan"
	},
	{
		"name": "花青",
		"hex": "#2376b7",
		"pinyin": "huaqing"
	},
	{
		"name": "睛蓝",
		"hex": "#5698c3",
		"pinyin": "jinglan"
	},
	{
		"name": "虹蓝",
		"hex": "#2177b8",
		"pinyin": "honglan"
	},
	{
		"name": "湖水蓝",
		"hex": "#b0d5df",
		"pinyin": "hushuilan"
	},
	{
		"name": "秋波蓝",
		"hex": "#8abcd1",
		"pinyin": "qiubolan"
	},
	{
		"name": "涧石蓝",
		"hex": "#66a9c9",
		"pinyin": "jianshilan"
	},
	{
		"name": "潮蓝",
		"hex": "#2983bb",
		"pinyin": "chaolan"
	},
	{
		"name": "群青",
		"hex": "#1772b4",
		"pinyin": "qunqing"
	},
	{
		"name": "霁青",
		"hex": "#63bbd0",
		"pinyin": "jiqing"
	},
	{
		"name": "碧青",
		"hex": "#5cb3cc",
		"pinyin": "biqing"
	},
	{
		"name": "宝石蓝",
		"hex": "#2486b9",
		"pinyin": "baoshilan"
	},
	{
		"name": "天蓝",
		"hex": "#1677b3",
		"pinyin": "tianlan"
	},
	{
		"name": "柏林蓝",
		"hex": "#126bae",
		"pinyin": "bolinlan"
	},
	{
		"name": "海青",
		"hex": "#22a2c3",
		"pinyin": "haiqing"
	},
	{
		"name": "钴蓝",
		"hex": "#1a94bc",
		"pinyin": "gulan"
	},
	{
		"name": "鸢尾蓝",
		"hex": "#158bb8",
		"pinyin": "yuanweilan"
	},
	{
		"name": "牵牛花蓝",
		"hex": "#1177b0",
		"pinyin": "qianniuhualan"
	},
	{
		"name": "飞燕草蓝",
		"hex": "#0f59a4",
		"pinyin": "feiyancaolan"
	},
	{
		"name": "品蓝",
		"hex": "#2b73af",
		"pinyin": "pinlan"
	},
	{
		"name": "银鱼白",
		"hex": "#cdd1d3",
		"pinyin": "yinyubai"
	},
	{
		"name": "安安蓝",
		"hex": "#3170a7",
		"pinyin": "ananlan"
	},
	{
		"name": "鱼尾灰",
		"hex": "#5e616d",
		"pinyin": "yuweihui"
	},
	{
		"name": "鲸鱼灰",
		"hex": "#475164",
		"pinyin": "jingyuhui"
	},
	{
		"name": "海参灰",
		"hex": "#fffefa",
		"pinyin": "haishenhui"
	},
	{
		"name": "沙鱼灰",
		"hex": "#35333c",
		"pinyin": "shayuhui"
	},
	{
		"name": "钢蓝",
		"hex": "#0f1423",
		"pinyin": "ganglan"
	},
	{
		"name": "云水蓝",
		"hex": "#baccd9",
		"pinyin": "yunshuilan"
	},
	{
		"name": "晴山蓝",
		"hex": "#8fb2c9",
		"pinyin": "qingshanlan"
	},
	{
		"name": "靛青",
		"hex": "#1661ab",
		"pinyin": "dianqing"
	},
	{
		"name": "大理石灰",
		"hex": "#c4cbcf",
		"pinyin": "dalishihui"
	},
	{
		"name": "海涛蓝",
		"hex": "#15559a",
		"pinyin": "haitaolan"
	},
	{
		"name": "蝶翅蓝",
		"hex": "#4e7ca1",
		"pinyin": "diechilan"
	},
	{
		"name": "海军蓝",
		"hex": "#346c9c",
		"pinyin": "haijunlan"
	},
	{
		"name": "水牛灰",
		"hex": "#2f2f35",
		"pinyin": "shuiniuhui"
	},
	{
		"name": "牛角灰",
		"hex": "#2d2e36",
		"pinyin": "niujiaohui"
	},
	{
		"name": "燕颔蓝",
		"hex": "#131824",
		"pinyin": "yanhanlan"
	},
	{
		"name": "云峰白",
		"hex": "#d8e3e7",
		"pinyin": "yunfengbai"
	},
	{
		"name": "井天蓝",
		"hex": "#c3d7df",
		"pinyin": "jingtianlan"
	},
	{
		"name": "云山蓝",
		"hex": "#2f90b9",
		"pinyin": "yunshanlan"
	},
	{
		"name": "釉蓝",
		"hex": "#1781b5",
		"pinyin": "youlan"
	},
	{
		"name": "鸥蓝",
		"hex": "#c7d2d4",
		"pinyin": "oulan"
	},
	{
		"name": "搪磁蓝",
		"hex": "#11659a",
		"pinyin": "tangcilan"
	},
	{
		"name": "月影白",
		"hex": "#c0c4c3",
		"pinyin": "yueyingbai"
	},
	{
		"name": "星灰",
		"hex": "#b2bbbe",
		"pinyin": "xinghui"
	},
	{
		"name": "淡蓝灰",
		"hex": "#5e7987",
		"pinyin": "danlanhui"
	},
	{
		"name": "鷃蓝",
		"hex": "#144a74",
		"pinyin": "yanlan"
	},
	{
		"name": "嫩灰",
		"hex": "#74787a",
		"pinyin": "nenhui"
	},
	{
		"name": "战舰灰",
		"hex": "#495c69",
		"pinyin": "zhanjianhui"
	},
	{
		"name": "瓦罐灰",
		"hex": "#47484c",
		"pinyin": "waguanhui"
	},
	{
		"name": "青灰",
		"hex": "#2b333e",
		"pinyin": "qinghui"
	},
	{
		"name": "鸽蓝",
		"hex": "#1c2938",
		"pinyin": "gelan"
	},
	{
		"name": "钢青",
		"hex": "#142334",
		"pinyin": "gangqing"
	},
	{
		"name": "暗蓝",
		"hex": "#101f30",
		"pinyin": "anlan"
	},
	{
		"name": "月白",
		"hex": "#eef7f2",
		"pinyin": "yuebai"
	},
	{
		"name": "海天蓝",
		"hex": "#c6e6e8",
		"pinyin": "haitianlan"
	},
	{
		"name": "清水蓝",
		"hex": "#93d5dc",
		"pinyin": "qingshuilan"
	},
	{
		"name": "瀑布蓝",
		"hex": "#51c4d3",
		"pinyin": "pubulan"
	},
	{
		"name": "蔚蓝",
		"hex": "#29b7cb",
		"pinyin": "weilan"
	},
	{
		"name": "孔雀蓝",
		"hex": "#0eb0c9",
		"pinyin": "kongquelan"
	},
	{
		"name": "甸子蓝",
		"hex": "#10aec2",
		"pinyin": "dianzilan"
	},
	{
		"name": "石绿",
		"hex": "#57c3c2",
		"pinyin": "shilv"
	},
	{
		"name": "竹篁绿",
		"hex": "#b9dec9",
		"pinyin": "zhuhuanglv"
	},
	{
		"name": "粉绿",
		"hex": "#83cbac",
		"pinyin": "fenlv"
	},
	{
		"name": "美蝶绿",
		"hex": "#12aa9c",
		"pinyin": "meidielv"
	},
	{
		"name": "毛绿",
		"hex": "#66c18c",
		"pinyin": "maolv"
	},
	{
		"name": "蔻梢绿",
		"hex": "#5dbe8a",
		"pinyin": "koushaolv"
	},
	{
		"name": "麦苗绿",
		"hex": "#55bb8a",
		"pinyin": "maimiaolv"
	},
	{
		"name": "蛙绿",
		"hex": "#45b787",
		"pinyin": "walv"
	},
	{
		"name": "铜绿",
		"hex": "#2bae85",
		"pinyin": "tonglv"
	},
	{
		"name": "竹绿",
		"hex": "#1ba784",
		"pinyin": "zhulv"
	},
	{
		"name": "蓝绿",
		"hex": "#12a182",
		"pinyin": "lanlv"
	},
	{
		"name": "穹灰",
		"hex": "#c4d7d6",
		"pinyin": "qionghui"
	},
	{
		"name": "翠蓝",
		"hex": "#1e9eb3",
		"pinyin": "cuilan"
	},
	{
		"name": "胆矾蓝",
		"hex": "#0f95b0",
		"pinyin": "danfanlan"
	},
	{
		"name": "樫鸟蓝",
		"hex": "#1491a8",
		"pinyin": "jianniaolan"
	},
	{
		"name": "闪蓝",
		"hex": "#7cabb1",
		"pinyin": "shanlan"
	},
	{
		"name": "冰山蓝",
		"hex": "#a4aca7",
		"pinyin": "bingshanlan"
	},
	{
		"name": "虾壳青",
		"hex": "#869d9d",
		"pinyin": "xiakeqing"
	},
	{
		"name": "晚波蓝",
		"hex": "#648e93",
		"pinyin": "wanbolan"
	},
	{
		"name": "蜻蜓蓝",
		"hex": "#3b818c",
		"pinyin": "qingtinglan"
	},
	{
		"name": "玉鈫蓝",
		"hex": "#126e82",
		"pinyin": "yuqinlan"
	},
	{
		"name": "垩灰",
		"hex": "#737c7b",
		"pinyin": "ehui"
	},
	{
		"name": "夏云灰",
		"hex": "#617172",
		"pinyin": "xiayunhui"
	},
	{
		"name": "苍蓝",
		"hex": "#134857",
		"pinyin": "canglan"
	},
	{
		"name": "黄昏灰",
		"hex": "#474b4c",
		"pinyin": "huanghunhui"
	},
	{
		"name": "灰蓝",
		"hex": "#21373d",
		"pinyin": "huilan"
	},
	{
		"name": "深灰蓝",
		"hex": "#132c33",
		"pinyin": "shenhuilan"
	},
	{
		"name": "玉簪绿",
		"hex": "#a4cab6",
		"pinyin": "yuzanlv"
	},
	{
		"name": "青矾绿",
		"hex": "#2c9678",
		"pinyin": "qingfanlv"
	},
	{
		"name": "草原远绿",
		"hex": "#9abeaf",
		"pinyin": "caoyuanyuanlv"
	},
	{
		"name": "梧枝绿",
		"hex": "#69a794",
		"pinyin": "wuzhilv"
	},
	{
		"name": "浪花绿",
		"hex": "#92b3a5",
		"pinyin": "langhualv"
	},
	{
		"name": "海王绿",
		"hex": "#248067",
		"pinyin": "haiwanglv"
	},
	{
		"name": "亚丁绿",
		"hex": "#428675",
		"pinyin": "yadinglv"
	},
	{
		"name": "镍灰",
		"hex": "#9fa39a",
		"pinyin": "niehui"
	},
	{
		"name": "明灰",
		"hex": "#8a988e",
		"pinyin": "minghui"
	},
	{
		"name": "淡绿灰",
		"hex": "#70887d",
		"pinyin": "danlvhui"
	},
	{
		"name": "飞泉绿",
		"hex": "#497568",
		"pinyin": "feiquanlv"
	},
	{
		"name": "狼烟灰",
		"hex": "#5d655f",
		"pinyin": "langyanhui"
	},
	{
		"name": "绿灰",
		"hex": "#314a43",
		"pinyin": "lvhui"
	},
	{
		"name": "苍绿",
		"hex": "#223e36",
		"pinyin": "canglv"
	},
	{
		"name": "深海绿",
		"hex": "#1a3b32",
		"pinyin": "shenhailv"
	},
	{
		"name": "长石灰",
		"hex": "#363433",
		"pinyin": "changshihui"
	},
	{
		"name": "苷蓝绿",
		"hex": "#1f2623",
		"pinyin": "ganlanlv"
	},
	{
		"name": "莽丛绿",
		"hex": "#141e1b",
		"pinyin": "mangconglv"
	},
	{
		"name": "淡翠绿",
		"hex": "#c6dfc8",
		"pinyin": "dancuilv"
	},
	{
		"name": "明绿",
		"hex": "#9eccab",
		"pinyin": "minglv"
	},
	{
		"name": "田园绿",
		"hex": "#68b88e",
		"pinyin": "tianyuanlv"
	},
	{
		"name": "翠绿",
		"hex": "#20a162",
		"pinyin": "cuilv"
	},
	{
		"name": "淡绿",
		"hex": "#61ac85",
		"pinyin": "danlv"
	},
	{
		"name": "葱绿",
		"hex": "#40a070",
		"pinyin": "conglv"
	},
	{
		"name": "孔雀绿",
		"hex": "#229453",
		"pinyin": "kongquelv"
	},
	{
		"name": "艾绿",
		"hex": "#cad3c3",
		"pinyin": "ailv"
	},
	{
		"name": "蟾绿",
		"hex": "#3c9566",
		"pinyin": "chanlv"
	},
	{
		"name": "宫殿绿",
		"hex": "#20894d",
		"pinyin": "gongdianlv"
	},
	{
		"name": "松霜绿",
		"hex": "#83a78d",
		"pinyin": "songshuanglv"
	},
	{
		"name": "蛋白石绿",
		"hex": "#579572",
		"pinyin": "danbaishilv"
	},
	{
		"name": "薄荷绿",
		"hex": "#207f4c",
		"pinyin": "bohelv"
	},
	{
		"name": "瓦松绿",
		"hex": "#6e8b74",
		"pinyin": "wasonglv"
	},
	{
		"name": "荷叶绿",
		"hex": "#1a6840",
		"pinyin": "heyelv"
	},
	{
		"name": "田螺绿",
		"hex": "#5e665b",
		"pinyin": "tianluolv"
	},
	{
		"name": "白屈菜绿",
		"hex": "#485b4d",
		"pinyin": "baiqucailv"
	},
	{
		"name": "河豚灰",
		"hex": "#393733",
		"pinyin": "hetunhui"
	},
	{
		"name": "蒽油绿",
		"hex": "#373834",
		"pinyin": "enyoulv"
	},
	{
		"name": "槲寄生绿",
		"hex": "#2b312c",
		"pinyin": "hujishenglv"
	},
	{
		"name": "云杉绿",
		"hex": "#15231b",
		"pinyin": "yunshanlv"
	},
	{
		"name": "嫩菊绿",
		"hex": "#f0f5e5",
		"pinyin": "nenjulv"
	},
	{
		"name": "艾背绿",
		"hex": "#dfecd5",
		"pinyin": "aibeilv"
	},
	{
		"name": "嘉陵水绿",
		"hex": "#add5a2",
		"pinyin": "jialingshuilv"
	},
	{
		"name": "玉髓绿",
		"hex": "#41b349",
		"pinyin": "yusuilv"
	},
	{
		"name": "鲜绿",
		"hex": "#43b244",
		"pinyin": "xianlv"
	},
	{
		"name": "宝石绿",
		"hex": "#41ae3c",
		"pinyin": "baoshilv"
	},
	{
		"name": "海沬绿",
		"hex": "#e2e7bf",
		"pinyin": "haimeilv"
	},
	{
		"name": "姚黄",
		"hex": "#d0deaa",
		"pinyin": "yaohuang"
	},
	{
		"name": "橄榄石绿",
		"hex": "#b2cf87",
		"pinyin": "ganlanshilv"
	},
	{
		"name": "水绿",
		"hex": "#8cc269",
		"pinyin": "shuilv"
	},
	{
		"name": "芦苇绿",
		"hex": "#b7d07a",
		"pinyin": "luweilv"
	},
	{
		"name": "槐花黄绿",
		"hex": "#d2d97a",
		"pinyin": "huaihuahuanglv"
	},
	{
		"name": "苹果绿",
		"hex": "#bacf65",
		"pinyin": "pingguolv"
	},
	{
		"name": "芽绿",
		"hex": "#96c24e",
		"pinyin": "yalv"
	},
	{
		"name": "蝶黄",
		"hex": "#e2d849",
		"pinyin": "diehuang"
	},
	{
		"name": "橄榄黄绿",
		"hex": "#bec936",
		"pinyin": "ganlanhuanglv"
	},
	{
		"name": "鹦鹉绿",
		"hex": "#5bae23",
		"pinyin": "yingwulv"
	},
	{
		"name": "油绿",
		"hex": "#253d24",
		"pinyin": "youlv"
	},
	{
		"name": "象牙白",
		"hex": "#fffef8",
		"pinyin": "xiangyabai"
	},
	{
		"name": "汉白玉",
		"hex": "#f8f4ed",
		"pinyin": "hanbaiyu"
	},
	{
		"name": "雪白",
		"hex": "#fffef9",
		"pinyin": "xuebai"
	},
	{
		"name": "鱼肚白",
		"hex": "#f7f4ed",
		"pinyin": "yudubai"
	},
	{
		"name": "珍珠灰",
		"hex": "#e4dfd7",
		"pinyin": "zhenzhuhui"
	},
	{
		"name": "浅灰",
		"hex": "#dad4cb",
		"pinyin": "qianhui"
	},
	{
		"name": "铅灰",
		"hex": "#bbb5ac",
		"pinyin": "qianhui"
	},
	{
		"name": "中灰",
		"hex": "#bbb5ac",
		"pinyin": "zhonghui"
	},
	{
		"name": "瓦灰",
		"hex": "#867e76",
		"pinyin": "wahui"
	},
	{
		"name": "夜灰",
		"hex": "#847c74",
		"pinyin": "yehui"
	},
	{
		"name": "雁灰",
		"hex": "#80766e",
		"pinyin": "yanhui"
	},
	{
		"name": "深灰",
		"hex": "#81776e",
		"pinyin": "shenhui"
	}
];

//#endregion
//#region src/index.js
var Color = class extends fn_default {
	constructor() {
		super();
		this.children = colors_default;
	}
};
var src_default = Color;

//#endregion
module.exports = src_default;